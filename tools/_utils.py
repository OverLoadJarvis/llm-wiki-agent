...
def is_db_mode() -> bool:
    return getattr(_ctx, 'db', None) is not None and getattr(_ctx, 'project_id', None) is not None


def get_storage():
    return (getattr(_ctx, 'db', None), getattr(_ctx, 'project_id', None))
...
def read_file(path: Path) -> str:
    db = getattr(_ctx, 'db', None)
    pid = getattr(_ctx, 'project_id', None)
    if db is not None:
        rel = _to_db_path(path)
        content = db.get_file_text_by_path(pid, rel)
        return content or ""
...
def read_file_bytes(path: Path) -> bytes:
    ...
    if db is not None:
        rel = _to_db_path(path)
        content = db.get_file_content_by_path(pid, rel)
        return content or b""
...
def file_exists(path: Path) -> bool:
    ...
    if db is not None:
        rel = _to_db_path(path)
        return db.get_file_by_path(pid, rel) is not None
    return path.exists()
...
def delete_file(path: Path):
    ...
    if db is not None:
        rel = _to_db_path(path)
        db.delete_file_by_path(pid, rel)
        return
    if path.exists():
        path.unlink()
...
def list_dir(path: Path, pattern: str = "*.md") -> list[Path]:
    ...
    if db is not None:
        ...
        files = db.list_files(pid, prefix=prefix)
        import fnmatch
...
def all_wiki_pages() -> list[Path]:
    ...
    pid = getattr(_ctx, 'project_id', None)
    if db is not None:
        files = db.list_files(pid, prefix="wiki/")
        return [Path(f["relative_path"]) for f in files
